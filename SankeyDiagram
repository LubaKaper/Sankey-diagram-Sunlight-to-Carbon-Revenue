  ...
  return (
    <>
      {[0, 1, 2, 3, 4, 5, 6, 7].map(i => (
        <circle key={i} r="4" fill={color} opacity="0">
          <animateMotion
            dur={`${2.8 + i * 0.35}s`}
  ...
  return (
    ...
    >
      ...
      <div style={{ position: "absolute", top: 0, left: 0, right: 0, bottom: 0, pointerEvents: "none" }}>
        <div style={{ position: "absolute", top: "-8%", left: "5%", width: "30%", height: "40%", borderRadius: "50%", background: "radial-gradient(circle, rgba(232,168,34,0.06) 0%, transparent 60%)", filter: "blur(80px)" }} />
        <div style={{ position: "absolute", bottom: "5%", right: "2%", width: "28%", height: "35%", borderRadius: "50%", background: "radial-gradient(circle, rgba(11,125,218,0.05) 0%, transparent 60%)", filter: "blur(70px)" }} />
        <div style={{ position: "absolute", top: "35%", left: "40%", width: "22%", height: "28%", borderRadius: "50%", background: "radial-gradient(circle, rgba(46,158,90,0.04) 0%, transparent 60%)", filter: "blur(60px)" }} />
        <svg width="100%" height="100%" style={{ position: "absolute", top: 0, left: 0, opacity: 0.01 }}>
          <defs><pattern id="gg" width="60" height="60" patternUnits="userSpaceOnUse"><path d="M 60 0 L 0 0 0 60" fill="none" stroke="#fff" strokeWidth="0.5" /></pattern></defs>
      ...
      >
        ...
        <text
          x={W / 2} y={H * 0.065 + titleSize * 1.2 + subtitleSize * 1.6}
          textAnchor="middle"
        ...
        {(() => {
          ...
          return (
            <g>
              {/* Separator line */}
              <line x1={PAD_X} y1={stripY - 30} x2={W - PAD_X} y2={stripY - 30} stroke="rgba(255,255,255,0.06)" strokeWidth="1" />
              {steps.map((s, i) => {
                const cx = PAD_X + i * stepW + stepW / 2;
                return (
                  <g key={i}>
                    <text x={cx} y={stripY} textAnchor="middle" fill={s.color} fontSize={Math.max(14, W * 0.015)} fontWeight="700" fontFamily="'JetBrains Mono', monospace" letterSpacing="-0.01em">{s.val}</text>
                    <text x={cx} y={stripY + Math.max(14, W * 0.012)} textAnchor="middle" fill="rgba(255,255,255,0.2)" fontSize={Math.max(8, W * 0.007)} fontWeight="600" fontFamily="'Outfit', sans-serif" letterSpacing="0.12em">{s.label}</text>
                    {i < totalSteps - 1 && i % 2 === 0 && (
                      <text x={cx + stepW / 2 + stepW / 2} y={stripY} textAnchor="middle" fill="rgba(255,255,255,0.15)" fontSize={arrowSize} fontWeight="300">→</text>
                    )}
                  </g>
                );
              })}
        ...
        <g>
          <rect x={20} y={H - 40} width={22} height={22} rx={5} fill="rgba(232,168,34,0.15)" stroke="rgba(232,168,34,0.3)" strokeWidth="1" />
          <text x={31} y={H - 24} textAnchor="middle" fill="#E8A822" fontSize="11" fontWeight="800" fontFamily="'JetBrains Mono', monospace">C</text>
          <text x={52} y={H - 24} fill="rgba(255,255,255,0.4)" fontSize="12" fontWeight="600" fontFamily="'Outfit', sans-serif">CarbonLens</text>
        </g>

        {/* Confidential — bottom right */}
        <text x={W - 20} y={H - 24} textAnchor="end" fill="rgba(255,255,255,0.15)" fontSize="10" fontFamily="'JetBrains Mono', monospace">Confidential</text>
      </svg>
    </div>
  );
}
